...
class MarketplaceController extends AbstractController
{
    ...
    // ── Helpers based on authenticated user ──────────────────────────────────
    ...
    private function getCurrentUserId(): string
    {
        ...
        if (!$user instanceof User || !$user->getId()) {
            throw $this->createAccessDeniedException('You must be logged in to access marketplace features.');
        }
    ...
    public function editProduct(int $id, Request $request): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            $stmt->execute([
                ...
                $request->request->get('image', $product['image']), $id, $this->getCurrentUserId(),
            ]);
    ...
    public function confirmOrder(Request $request): Response
    {
        ...
        try {
            ...
            $stmt->execute([$this->getCurrentUserId(), $total, $deliveryStatus, $paymentMethod]);
...
public function stripeSuccess(Request $request): Response
{
    ...
    if ($session && $session->payment_status === 'paid') {
        ...
        try {
            ...
            ");
            $stmt->execute([$this->getCurrentUserId(), $total]);

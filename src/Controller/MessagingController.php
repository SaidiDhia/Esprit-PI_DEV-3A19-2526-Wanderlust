...
public function index(Request $request, EntityManagerInterface $em): Response
{
    ...
    public function newConversation(Request $request, EntityManagerInterface $em): Response
    {
        if ($request->isMethod('POST')) {
            ...
            if (empty($name)) {
                $this->addFlash('error', 'Ã¢Å¡Â Ã¯Â¸Â Conversation name cannot be empty.');
            ...
            if ($type === 'GROUP' && count($participantEmails) < 2) {
                $this->addFlash('error', 'Ã¢Å¡Â Ã¯Â¸Â Group conversation needs at least 2 participants.');
                return $this->redirectToRoute('messaging_conversation_new');
            }
            
            // For PERSONAL, need exactly 1 participant
            if ($type === 'PERSONAL' && count($participantEmails) != 1) {
                $this->addFlash('error', 'Ã¢Å¡Â Ã¯Â¸Â Personal conversation needs exactly 1 participant.');
                return $this->redirectToRoute('messaging_conversation_new');
            }
            ...
            foreach ($participantEmails as $email) {
                ...
                if (!$participant) {
                    $this->addFlash('error', 'Ã¢Å¡Â Ã¯Â¸Â User not found with email: ' . $email);
                    return $this->redirectToRoute('messaging_conversation_new');
                }

                $participantId = (string) $participant['id'];
                if ($participantId === $currentUserId) {
                    $this->addFlash('error', 'Ã¢Å¡Â Ã¯Â¸Â You cannot add yourself as a participant.');
                    return $this->redirectToRoute('messaging_conversation_new');
                }
            ...
            if ($type === 'GROUP' && count($participantIds) < 2) {
                $this->addFlash('error', 'Ã¢Å¡Â Ã¯Â¸Â Group conversation needs at least 2 other participants.');
                return $this->redirectToRoute('messaging_conversation_new');
            }

            if ($type === 'PERSONAL' && count($participantIds) !== 1) {
                $this->addFlash('error', 'Ã¢Å¡Â Ã¯Â¸Â Personal conversation needs exactly 1 other participant.');
                return $this->redirectToRoute('messaging_conversation_new');
            }
    ...
    public function sendMessage(Request $request, EntityManagerInterface $em): Response
    {
        ...
        if ($conversationId <= 0) {
            $this->addFlash('error', 'Ã¢Å¡Â Ã¯Â¸Â Conversation is required.');
            return $this->redirectToRoute('app_messaging');
        }

        if ($content === '') {
            $this->addFlash('error', 'Ã¢Å¡Â Ã¯Â¸Â Please write a message before sending.');
...
public function addParticipant(int $id, Request $request, EntityManagerInterface $em): Response
{
    ...
    if (!$newUser) {
        ...
        if ($existing) {
            if ($existing['is_active'] == 1) {
                ...
                $stmt->bindValue('content', 'Ã°Å¸â€˜Â¤ ' . $email . ' was re-added to the conversation');
                $stmt->executeStatement();
    ...
    public function removeParticipant(int $id, string $userId, EntityManagerInterface $em): Response
    {
        ...
        $stmt->bindValue('convId', $id);
        $stmt->bindValue('senderId', $this->getCurrentUserId());
        $stmt->bindValue('content', 'Ã°Å¸â€˜Â¤ A participant was removed from the conversation');
        $stmt->executeStatement();

...
class Posts
{
    ...
    /**
     * Relation with User (author of the post)
     */
    ...
    public function addCommentaire(Commentaires $commentaire): self { if (!$this->commentaires->contains($commentaire)) { $this->commentaires->add($commentaire); $commentaire->setPost($this); } return $this; }
    public function removeCommentaire(Commentaires $commentaire): self { $this->commentaires->removeElement($commentaire); return $this; }

    public function getReactions(): Collection { return $this->reactions; }
    public function addReaction(Reactions $reaction): self { if (!$this->reactions->contains($reaction)) { $this->reactions->add($reaction); $reaction->setPost($this); } return $this; }
    ...
    public function addSavedByUser(PostsSauvegardes $save): self { if (!$this->savedByUsers->contains($save)) { $this->savedByUsers->add($save); $save->setPost($this); } return $this; }
    public function removeSavedByUser(PostsSauvegardes $save): self { $this->savedByUsers->removeElement($save); return $this; }
}